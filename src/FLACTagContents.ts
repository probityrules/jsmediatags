...
class FLACTagContents {
  ...
  toArray(): ByteArray {
    ...
    return this._blocks.reduce(function (array: ByteArray, block) {
  ...
  static createStreamBlock(): MetadataBlock {
    let data = ([] as ByteArray).concat([0x00, 0x00, 0x22], Array(34).fill(0x00));
...
class MetadataBlock {
  ...
  toArray(): ByteArray {
    return ([this._type + (this._final ? 128 : 0)] as ByteArray).concat(this._data);
  }
}
